...
Companion to `how`. `how` tells you what the code does and, in History mode, why it's shaped that way. Blast radius tells you what it breaks somewhere else.
...
1. Read the change. The diff, the symbols it adds, changes, and deletes, and what it now does differently, including the part the diff doesn't spell out. Pull the history behind it:

   ```bash
   git diff main...HEAD
   git log --oneline -20 -- <file>
   gh pr view <number> --json title,body,comments,reviews   # when there is a PR
   ```
