Synthesize three reviewers' findings from the active transcript into skill edits, backlog items, or rejections. Do not modify files; the parent applies the Accepted list after user approval. Use `gh`, `git`, and any MCP tool available to verify a finding (a PR, a Cloudflare log, a doc).
...
- Structural-mechanism check: route to Backlog when a lint rule, script, metadata flag, or runtime check already enforces the rule or could enforce it cheaply. Skill prose is for things mechanisms cannot enforce. Rules files (`~/.claude/rules/*.md`), playbooks, principles, and a project's CLAUDE.md are valid routing targets alongside skills; pick the one the parent actually loaded.
...
For each item, describe the pattern, what was hit, and the suggested mechanism. The parent appends each to `~/dev/personal/my-skills/BACKLOG.md`.
