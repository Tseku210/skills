...
- Structural-mechanism check: route to Backlog when a lint rule, script, metadata flag, or runtime check already enforces the rule or could enforce it cheaply. Skill prose is for things mechanisms cannot enforce. Rules files (`~/.claude/rules/*.md`) and a project's CLAUDE.md are valid routing targets alongside skills; pick the one the parent actually loaded.
