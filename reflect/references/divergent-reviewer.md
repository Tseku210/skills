...
Do not modify files in the repo. Use `gh`, `git`, and any MCP tool available (Cloudflare observability, Figma, context7 docs) to look up context referenced in the transcript. Read code, fetch PRs, query logs, but do not write code, edit skills, or commit. The parent agent applies edits based on your output.
