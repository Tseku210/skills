...
3. Migrate one component at a time. Shared primitives (tokens, Container, type scale) migrate first as a blocking phase. Parallelize across worktrees only when there are several independent components and the laptop can carry it; otherwise serial.
