...
Ask the user for the PRD Linear issue identifier or URL.

If the PRD is not already in your context window, fetch it with the `get_issue` Linear MCP tool.
...
For each approved slice, create a Linear issue using the `create_issue` MCP tool. Use the issue body template below.

Create issues in dependency order (blockers first) so you can reference real issue identifiers in the "Blocked by" field.

<issue-template>
## Parent PRD
<prd-issue-identifier> (e.g. PER-12)
