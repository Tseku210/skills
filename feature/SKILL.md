---
name: feature
description: "Own new or changed behavior that has no tickets: learn the subsystem, settle the design, name the data shape, delegate the code, verify on the real surface, open a PR. Use when the user asks to add, build or change behavior in this session. Ticketed work goes to /execute-mode instead."
---
...
3. Write the throughput plan as four todo items. A dimension that genuinely does not apply (single file, no fan-out) keeps its item with `n/a: <reason>` rather than being dropped:
   ...
   - **Smallest safe decomposition.** If one worker is best, name why. Usually it is.
...
7. If the design is contested, run `mattpocock-skills:code-review` before shipping. The built-in `/code-review` is a different tool.
8. Run **Opening a PR** (`~/dev/personal/my-skills/playbooks/opening-a-pr.md`).
